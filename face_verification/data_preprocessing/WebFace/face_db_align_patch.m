...
function crop = patch_crop(img_resize, ffp, patch_size, ang, img_size, img_rot_size, resize_scale)
    ...
    [hh ww c] = size(img_resize);
    
    crop_y = max(pp(2) - patch_size / 2 + 1, 1);
    crop_y_end = min(pp(2) + patch_size / 2, hh);
    crop_x = max(pp(1) - patch_size / 2 + 1, 1);
    crop_x_end = min(pp(1) + patch_size / 2, ww);
